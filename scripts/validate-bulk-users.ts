...
const csvPath = ".local/bulk-user-bootstrap-preview.csv";
...
type CsvRow = Record<ColumnName, string>;
...
function main() {
  if (!existsSync(absoluteCsvPath)) {
    console.error(
      `Bulk user CSV not found at ${csvPath}. This file is local-only and must be created before running the dry-run validator.`,
    );
    process.exit(1);
  }
  ...
  printSummary(rows, workers, checklist, issues);
...
function toRows(records: string[][], issues: ValidationIssue[]) {
  return records.flatMap((record) => {
    ...
    return [
      Object.fromEntries(
        expectedHeader.map((column, index) => [column, record[index] ?? ""]),
      ) as CsvRow,
    ];
  });
}
...
function checkUniqueEmails(rows: CsvRow[], issues: ValidationIssue[]) {
...
function checkWorkerGids(workers: CsvRow[], issues: ValidationIssue[]) {
...
function checkValidTiers(rows: CsvRow[], issues: ValidationIssue[]) {
...
function checkValidShifts(workers: CsvRow[], issues: ValidationIssue[]) {
...
function checkCustomerServiceTier(workers: CsvRow[], issues: ValidationIssue[]) {
...
function checkNonCustomerServiceTier(workers: CsvRow[], issues: ValidationIssue[]) {
...
function checkFlexibleRows(workers: CsvRow[], issues: ValidationIssue[]) {
...
function checkOwnerWorkerFields(owners: CsvRow[], issues: ValidationIssue[]) {
...
function checkAvatarInitials(rows: CsvRow[], issues: ValidationIssue[]) {
...
function checkPasswordColumn(rows: CsvRow[], issues: ValidationIssue[]) {
...
function countBy(rows: CsvRow[], field: ColumnName) {
...
function rowRef(row: CsvRow): RowRef {
  return {
    name: row.name,
    email: row.email,
    gid: row.gid,
...
function printSummary(rows: CsvRow[], workers: CsvRow[], checklist: ChecklistItem[], issues: ValidationIssue[]) {
  console.log("Bulk user bootstrap dry-run validation");
  ...
  for (const issue of issues) {
    const row = issue.row ? ` name=${issue.row.name || "(blank)"} email=${issue.row.email || "(blank)"} gid=${issue.row.gid || "(blank)"}` : "";
