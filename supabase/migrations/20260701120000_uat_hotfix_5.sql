-- UAT Hotfix 5: Absolute Operational Day & Reset Records RPC

-- 1. Redefine apply_tracker_action_impl with strictly 6 hours offset date logic
...
begin
  ...
  -- Calculate v_attendance_date strictly using 6 hours offset from p_now
  v_attendance_date := (v_wib_timestamp - interval '6 hours')::date;
  ...
  if v_is_flexible then
    ...
    begin
      ...
      v_shift_end_date := v_cycle_date;
      if v_shift_end_minutes <= v_shift_start_minutes then
        v_shift_end_date := v_cycle_date + 1;
      end if;
...
-- 2. Redefine execute_cron_state_machine with shifted 6-hour WIB offset logic
    ...
    if not v_is_flexible then
      ...
      then
        ...
        begin
          ...
          -- Calculate v_attendance_date strictly shifted back by 6 hours from the shift start time
          v_attendance_date := (v_shift_starts_at - interval '6 hours')::date;
...
-- 3. Implement public.reset_records RPC restricted to owner
...
begin
  ...
  set
    ...
    last_source = 'records',
    last_source_action = 'records.reset',
