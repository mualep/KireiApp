-- Migration: Fix CANCEL_START for Active and Late Shifts with Record Reversion
-- Redefine app_private.apply_tracker_action_impl so CANCEL_START reverts work_late_seconds from worker_records if the cancelled shift started LATE.
...
-- 2. Redefine public.apply_tracker_action RPC wrapper with sqlerrm error code passthrough
