-- R3D-A Absensi correction DB/RPC foundation.

create table if not exists public.worker_absensi_corrections (
  ...
  before_source text,
  before_source_action text,
  after_source text not null default 'absensi',
  after_source_action text not null,
  before_attendance_is_canceled boolean not null default false,
  pending_days_delta integer not null default 0,
  sakit_days_delta integer not null default 0,
  alpha_count_delta integer not null default 0,
  cuti_stock_delta integer not null default 0,
  cuti_stock_before smallint,
  cuti_stock_after smallint,
  reason text not null,
  created_at timestamptz not null default now(),
  constraint worker_absensi_corrections_before_status_check check (
    before_status in ('none', 'hadir', 'cuti', 'sakit', 'pending', 'alpha')
  ),
  constraint worker_absensi_corrections_after_status_check check (
    after_status in ('hadir', 'cuti', 'sakit', 'pending', 'alpha')
  ),
  constraint worker_absensi_corrections_transition_check check (
  ...
  constraint worker_absensi_corrections_before_source_check check (
    before_source is null
    or before_source in ('tracker', 'absensi', 'cron', 'system')
  ),
  constraint worker_absensi_corrections_before_source_action_check check (
    before_source_action is null
    or before_source_action ~ '^[a-z][a-z0-9_.-]*$'
  ),
  constraint worker_absensi_corrections_after_source_check check (
    after_source = 'absensi'
  ),
  constraint worker_absensi_corrections_after_source_action_check check (
    after_source_action in (
  ...
  constraint worker_absensi_corrections_reason_check check (
    char_length(pg_catalog.btrim(reason)) between 1 and 500
  ),
  constraint worker_absensi_corrections_cuti_before_check check (
    cuti_stock_before is null or cuti_stock_before >= 0
  ),
  constraint worker_absensi_corrections_cuti_after_check check (
    cuti_stock_after is null or cuti_stock_after >= 0
  )
);

comment on table public.worker_absensi_corrections is
  'Immutable Absensi/Admin historical attendance correction ledger.';

create index if not exists worker_absensi_corrections_attendance_id_idx
  on public.worker_absensi_corrections (attendance_id);

create index if not exists worker_absensi_corrections_target_date_idx
  on public.worker_absensi_corrections (target_user_id, attendance_date desc);

create index if not exists worker_absensi_corrections_actor_created_idx
  on public.worker_absensi_corrections (actor_user_id, created_at desc);

alter table public.worker_absensi_corrections enable row level security;

revoke all on public.worker_absensi_corrections from anon, authenticated;
grant select on public.worker_absensi_corrections to authenticated;

create policy worker_absensi_corrections_select_admin_only
on public.worker_absensi_corrections
for select
to authenticated
...
comment on function app_private.apply_absensi_correction_impl(
  ...
  'Private Absensi/Admin historical attendance correction implementation with deterministic deltas, immutable ledger, and fail-closed audit.';
...
create or replace function public.apply_absensi_correction(
  p_target_user_id uuid,
  p_attendance_date date,
  p_before_status text,
  p_after_status text,
...
begin
  if v_actor_id is null then
    raise exception 'absensi.unauthenticated' using errcode = '42501';
  end if;
  ...
  return app_private.apply_absensi_correction_impl(
    v_actor_id,
    p_target_user_id,
    p_attendance_date,
    p_before_status,
    p_after_status,
    p_expected_attendance_id,
    p_expected_attendance_updated_at,
    p_reason,
...
comment on function public.apply_absensi_correction(uuid, date, text, text, uuid, timestamptz, text) is
  'Authenticated Owner/Admin RPC entrypoint for historical Absensi attendance correction.';
