-- Migration: Fix sync_absensi_edit_impl non-existent columns
-- 1. Set default values for source and source_action on worker_attendance to prevent not-null violations on insert
ALTER TABLE public.worker_attendance ALTER COLUMN source SET DEFAULT 'absensi';
ALTER TABLE public.worker_attendance ALTER COLUMN source_action SET DEFAULT 'absensi.edit_cell';

-- 2. Recreate sync_absensi_edit_impl using strictly existing columns
...
BEGIN
  ...
  -- 6. Upsert worker_attendance (Strictly conforming to existing columns, removing source, source_action, updated_by)
