...
CREATE INDEX IF NOT EXISTS worker_profiles_shift_idx ON public.worker_profiles(shift);
CREATE INDEX IF NOT EXISTS worker_attendance_user_date_idx ON public.worker_attendance(user_id, attendance_date);
CREATE INDEX IF NOT EXISTS worker_sp_logs_user_revoked_expires_idx ON public.worker_sp_logs(user_id, revoked_at, expires_at);

-- Replace issue_worker_sp_impl function to extend existing active SPs to the new expiry date
...
begin
  ...
  -- Roll forward expiry of all active SPs BEFORE inserting the new one (so the new one starts with correct date and we don't redundant update)
  update public.worker_sp_logs
...
-- Replace execute_cron_state_machine function with the AUTO_ALPHA_EXPIRE fix (only checking current shift expiry v_exp1)
