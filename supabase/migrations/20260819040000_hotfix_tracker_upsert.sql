-- Migration: Hotfix Tracker Records Upsert & Scheduled Attendance Partial Unique Index
-- 1. Fix cuti_stock_delta NOT NULL violation during worker_records upsert in app_private.apply_tracker_action_impl
-- 2. Drop strict unique constraint on scheduled_attendance(user_id, target_date) and replace with active partial unique index

-- 1. Scheduled Attendance Partial Unique Index
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    ...
    WHERE table_schema = 'public'
      AND table_name = 'scheduled_attendance'
      AND constraint_type = 'UNIQUE'
  LOOP
    EXECUTE format('ALTER TABLE public.scheduled_attendance DROP CONSTRAINT IF EXISTS %I', r.constraint_name);
  END LOOP;
END $$;

DROP INDEX IF EXISTS public.scheduled_attendance_user_id_target_date_key;
DROP INDEX IF EXISTS public.scheduled_attendance_user_target_date_unique;

CREATE UNIQUE INDEX IF NOT EXISTS idx_scheduled_attendance_active_unique
  ON public.scheduled_attendance (user_id, target_date)
  WHERE cancelled_at IS NULL AND applied_at IS NULL;


-- 2. Redefine app_private.apply_tracker_action_impl with explicit cuti_stock_delta = 0 in worker_records inserts
  ...
  IF v_action = 'START' THEN
    ...
    IF v_record_work_late_seconds > 0 THEN
      ...
      INSERT INTO public.worker_records (
        ...
        sakit_days,
        cuti_stock_snapshot,
        cuti_stock_delta, -- Explicitly specified to avoid NOT NULL violation on upsert
        last_source,
        last_source_action,
        created_at,
        updated_at
      ...
      VALUES (
        ...
        0, -- Explicit 0 for non-cuti action
        'tracker',
        v_source_action,
        p_now,
        p_now
