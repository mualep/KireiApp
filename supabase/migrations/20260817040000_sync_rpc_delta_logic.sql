-- Migration: Sync RPC CUTI Action Validation with V2 Delta Overrides
-- Redefine app_private.apply_tracker_action_impl so that CUTI action checks effective cuti stock (cuti_stock + cuti_stock_delta) from worker_records.
...
begin
  ...
  -- [CANCEL START OPERATIONAL DATE TARGET]
  if v_action = 'CANCEL_START' then
    v_attendance_date := coalesce(v_shift_active_date, v_attendance_date);
  end if;
  ...
  -- [CANCEL START GRACE PERIOD VALIDATION]
  if v_action = 'CANCEL_START' then
    if v_from_status <> 'on' or v_shift_active_started_at is null then
      raise exception 'tracker.invalid_transition' using errcode = '22023';
    end if;

    if pg_catalog.floor(pg_catalog.date_part('epoch', p_now - v_shift_active_started_at))::integer > 900 then
      raise exception 'tracker.invalid_transition' using errcode = '22023';
    end if;
  end if;
  ...
  v_audit_action := case
    ...
    when v_action = 'CANCEL_START' then 'tracker.cancel_start'
  end;
  ...
  if v_action = 'START' then
    ...
    -- Calculate if this START recorded late seconds into worker_records
    ...
    -- If late seconds were recorded, revert them from worker_records cleanly
    ...
    -- [V2 DELTA INTEGRATION] Fetch cuti_stock_delta from worker_records for this period_month
    select coalesce(wr.cuti_stock_delta, 0)
      into v_cuti_stock_delta_recorded
    from public.worker_records as wr
    where wr.user_id = p_target_user_id
      and wr.period_month = v_period_month;

    -- Validate against effective cuti stock (cuti_stock + cuti_stock_delta)
    if (v_cuti_stock_before + coalesce(v_cuti_stock_delta_recorded, 0)) <= 0 then
