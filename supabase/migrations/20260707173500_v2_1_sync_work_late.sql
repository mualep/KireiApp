-- Migration: Sync work_late_seconds on Absensi check-in/revert
...
BEGIN
  ...
  -- Handle the PL/pgSQL SELECT INTO NULL override when no row is found
  ...
  IF p_date = v_current_wib_date AND NOT coalesce(v_is_flexible, false) THEN
    -- Check shift start configurations
    ...
    THEN
      ...
      -- SCENARIO A: Transitioning away from Hadir -> Subtract late seconds if they were late
      ...
      -- SCENARIO B: Transitioning to Hadir -> Add late seconds if clock-in is late
  ...
  -- 8. If p_date = TODAY: Update worker_status operational state (One-Way Truth)
