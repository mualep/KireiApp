-- Migration: Support kompensasi_duration_mins and telat_izin_count in apply_records_delta_override_impl
...
BEGIN
  ...
  v_norm_field := CASE p_field_name
    ...
    WHEN 'kompensasi_override_duration_mins' THEN 'kompensasi_duration_mins'
    WHEN 'telat_izin_override_count' THEN 'telat_izin_count'
    ELSE p_field_name
  END;
  ...
  ELSIF v_norm_field = 'kompensasi_duration_mins' THEN
    SELECT kompensasi_duration_mins, kompensasi_delta_mins INTO v_existing_auto, v_existing_delta FROM public.worker_records WHERE user_id = p_target_user_id AND period_month = p_period_month FOR UPDATE;
  ELSIF v_norm_field = 'telat_izin_count' THEN
    SELECT telat_izin_count, telat_izin_delta INTO v_existing_auto, v_existing_delta FROM public.worker_records WHERE user_id = p_target_user_id AND period_month = p_period_month FOR UPDATE;
  ELSE
  ...
  ELSIF v_norm_field = 'kompensasi_duration_mins' THEN
    UPDATE public.worker_records SET kompensasi_delta_mins = v_new_delta, updated_at = p_now WHERE user_id = p_target_user_id AND period_month = p_period_month;
  ELSIF v_norm_field = 'telat_izin_count' THEN
    UPDATE public.worker_records SET telat_izin_delta = v_new_delta, updated_at = p_now WHERE user_id = p_target_user_id AND period_month = p_period_month;
  END IF;
