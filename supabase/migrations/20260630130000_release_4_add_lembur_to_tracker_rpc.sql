-- Re-implement app_private.apply_tracker_action_impl with support for manual overtime (LEMBUR / BATAL_LEMBUR)
...
declare
  ...
  -- Overtime (Lembur) local variables
  v_lembur_started_at timestamptz;
  v_lembur_secs integer;
  v_lembur_mins integer;
begin
  select u.tier
    into v_actor_tier
  ...
  v_audit_action := case
    ...
    when v_action = 'SELESAI' and v_from_status = 'lembur' then 'tracker.lembur_finish'
    ...
    when v_action = 'LEMBUR' then 'tracker.lembur_start'
    when v_action = 'BATAL_LEMBUR' then 'tracker.lembur_cancel'
  end;
  ...
  if v_action = 'START' then
    ...
    if not v_attendance_reused then
      ...
      if v_attendance_id is null then
        ...
        if v_existing_attendance_status = 'hadir' and not v_existing_attendance_canceled then
          v_attendance_reused := true;
          v_work_late_seconds_delta := 0;
        else
          raise exception 'tracker.attendance_conflict' using errcode = '23505';
    ...
    if v_from_status = 'lembur' then
      select ws.lembur_started_at into v_lembur_started_at from public.worker_status as ws where ws.user_id = p_target_user_id;
      if v_lembur_started_at is not null then
        v_lembur_secs := pg_catalog.floor(pg_catalog.date_part('epoch', p_now - v_lembur_started_at))::integer;
        if v_lembur_secs > 0 then
          v_lembur_mins := v_lembur_secs / 60;
          if v_lembur_mins > 0 then
            v_record_deltas := pg_catalog.jsonb_build_object('lembur_units', v_lembur_mins);
            ...
            set lembur_units = lembur_units + v_lembur_mins,
                updated_at = p_now
    ...
    returning ws.current_status, ws.version
      into v_to_status, v_to_version;
  elsif v_action = 'LEMBUR' then
    if v_from_status <> 'off' then
      raise exception 'tracker.invalid_transition' using errcode = '22023';
    end if;

    if not v_is_flexible and p_now < v_shift_ends_at then
      raise exception 'tracker.invalid_transition' using errcode = '22023';
    end if;

    update public.worker_status as ws
    set
      current_status = 'lembur',
      version = v_from_version + 1,
      lembur_started_at = p_now
    where ws.user_id = p_target_user_id
    returning ws.current_status, ws.version
      into v_to_status, v_to_version;
  elsif v_action = 'BATAL_LEMBUR' then
    if v_from_status <> 'lembur' then
      raise exception 'tracker.invalid_transition' using errcode = '22023';
    end if;
...
is 'Private SECURITY DEFINER tracker action implementation. R2C-D adds transaction-bound manual BREAK_LATE records when LANJUT closes an overdue break. Release 4 adds LEMBUR / BATAL_LEMBUR.';
