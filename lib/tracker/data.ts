  ...
  const [
    ...
    await Promise.all([
      ...
      supabase
        .from("worker_status")
        .select(
          "user_id,version,current_status,alpha_done,updated_at,break_started_at,break_timer_running,break_accumulated_secs,cuti_set_date,sakit_started_at,pending_started_at,break_late_recorded",
        )
        .in("user_id", userIds)
